...
> **Start at `HANDOFF.md`** — it is a one-page router.
>
> | You want to | Read |
> |---|---|
> | take a measurement | `bench/README.md` |
> | pick an instrument | `bench/choosing-an-smu.md` |
> | change the code | `docs/_index.md` |
> | add an SMU | `docs/workflow/adding-an-smu.md` |
> | know why old data disagrees with new | the instrument or experiment note in `docs/` |
> | write or run tests | `tests/README.md` |
>
> `docs/` is an Obsidian vault and also plain Markdown. Some pages under
> `bench/` and `docs/reference/` are generated by
> `tools/build_docs.py`; each says so in its first line.
...
true for `core/base_app.py` as well. `docs/architecture/core-modules.md`
has the reasoning under
"Run control".
...
`main.py`. Read `docs/rules/` first: landscape
...
the instrument's note in `docs/instruments/`.
...
- **Dual-SMU long bias** — not built. The `Keithley2401` driver exists, so it
  can be added if the requirement returns; see `docs/instruments/keysight-u2722a.md`.
...
an instrument. See `docs/workflow/adding-an-smu.md`.

Measured behaviour and per-instrument quirks are in `docs/instruments/`, which
...
consequences of that are written up in the experiment notes under
`docs/experiments/`.
