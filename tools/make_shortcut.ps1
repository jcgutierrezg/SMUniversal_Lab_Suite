...
.DESCRIPTION
    ...
    windowless `uvw.exe` and the `smu-lab-suite-gui` entry point:
        ...
        uvw run --directory <this checkout> smu-lab-suite-gui
    ...
    installs anything the update added.
...
Write-Host "Installing the suite's packages ..."
& $Uv run --directory $Repo python -c "import smuniversal_lab_suite.core.launcher"
if ($LASTEXITCODE -ne 0) {
    throw "Installing the packages failed - see the messages above."
}

$Arguments = "run --directory `"$Repo`" smu-lab-suite-gui"
