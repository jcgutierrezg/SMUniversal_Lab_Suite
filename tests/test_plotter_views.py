...
from plotter_files import FIXED_TITLE, fixed_run, iv_run, stored
...
CASES = [(v.key, "iv" if v.key.startswith("iv") else "fs")
         for v in views.VIEWS]


@pytest.mark.parametrize("view_key, family", CASES,
                         ids=[k for k, _ in CASES])
def test_every_view_draws_one_axis_per_panel(check, view_key, family):
    if family == "iv":
        series = _series([stored([iv_run("a"), iv_run("b", minutes=1)])])
    else:
        series = _series([_fixed([fixed_run("a", temperature=21.0),
