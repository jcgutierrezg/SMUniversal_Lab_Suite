...
| `core/gui/tooltips.py` | `Tooltips`, and the `tip()` helper panels call | every panel that explains itself | a panel that is obvious to whoever wrote it. Off until the "Show tooltips" box is ticked, one manager per window so two tooltips cannot share the screen, and the words live beside the widget rather than in a catalogue that would drift from it |
