...
| `core/identity.py` | `SampleRef`, `SampleRegistry`, `SESSION_ID` - stable ids for samples, runs, readings, results | calculation, run store, the session strip | provenance cites names, and names are not unique. The random tail is 64 bits, not 32: at a few hundred a day 32 bits collide about once in 260 years, not the "ten thousand" the module used to claim. A run id carries this process's `SESSION_ID` because the sequence number restarts with the app and the timestamp resolves to one second, so two launches — or two benches — produced the same first run id |
...
| `core/checkup/` | `Checkup` - the tiered commissioning probe, one module per tier plus `base` and `report`; `probe_levels_for()` - the levels it sources, reconciled against each driver's envelope | `tools/smu_checkup.py` | a driver is trusted because its tests pass, which is not the same claim. And one probe level for every instrument is a level some instrument cannot express - see [A test level the instrument cannot express](../faults/34-a-probe-the-instrument-cannot-express.md) |
| `core/readback.py` | The five states an instrument's answer about its own state can be in, and which of them is a pass | `BaseSMU.verify_compliance/verify_range/verify_power_limit`, `core/checkup/` | a setting is a request until something reads it back, and a readback nobody has verified is not evidence. See [A setting reported from the command that was sent](../faults/33-a-setting-never-read-back.md) |
| `core/provenance.py` | `head_commit()`, `firmware_from_idn()`, `describe()` - which code and which firmware a bench report describes | `tools/smu_checkup.py`, via `core/checkup/`'s report header | a finding is a claim about a version of the code *and* a version of the instrument, and reports recorded neither. Comparing a clean 2026-08-06 GSM-20H10 checkup against a six-failure 2026-08-18 one cost five rounds of hypotheses that a commit sha answers in one line |
| `core/addresses.py` | `GPIB_MAP`, `USB_IDS`, `SERIAL_IDS`, `keep()`, `label()`, `filtered()` | `core/gui/connection_panel.py` | the dropdown listed whatever a scan returned - TCPIP entries invented by a network scan, phantom motherboard COM ports, `::RAW` duplicates - and spelled the real ones `USB0::8580::125::gew852313::0::INSTR`. Bench wiring, not a fact about the instruments: a wrong entry mislabels a row and nothing more, because identity still comes from `*IDN?` at connect |
| `core/gui/tooltips.py` | `Tooltips`, and the `tip()` helper panels call | every panel that explains itself | a panel that is obvious to whoever wrote it. On by default and switched off by the "Show tooltips" box, shown at the pointer once it rests, one manager per window so two tooltips cannot share the screen, and the words live beside the widget rather than in a catalogue that would drift from it |
| `core/gui/help_link.py` | the [?] button, `SITE_URL`, `open_guide()` | the header strip of every window, and the plotter's toolbar | the user guide being one search away rather than one click. Each window opens its own page - an experiment's `GUIDE_PAGE`, the tab in front's in a window with two - and `tests/test_help_link.py` fails if one names a page the site does not have |
| `core/gui/equations.py` | `EquationsWindow`, `render()`, `number()` | `Experiment.show_equations()` | the arithmetic behind a result was only in the code and the docs. Typeset with matplotlib's mathtext, which needs no TeX installation; a formula too wide for the window is re-rendered smaller rather than cut off |
| `core/progress.py` | `remaining_seconds()`, `format_remaining()`, `seconds_per_reading()` - the time-left arithmetic behind every tab's progress bar | `experiments/base_experiment.py` (the one-second timer), each experiment's `estimate_run_seconds()` | nothing tells the operator how long a run will take. Pure numbers: the pace of the readings so far once a few are in, the experiment's estimate from its settings until then, shown to the second |
| `core/clamping.py` | `detect_clamping()` and its four tuning constants | `Experiment.check_clamping()`, called by every experiment before it commits | a run that ran into its compliance fits and averages as neatly as one that did not. Reads the data rather than asking the instrument, so it works where there is no compliance flag. The thresholds live here and nowhere else |
| `core/event_log.py` | JSON Lines record of how every run ended | `experiments/base_experiment.py`, via a sink injected into `RunController` | a cancelled or failed run would leave no trace once the window closed. Records *that* a run happened, never what it measured |
| `core/launcher.py` | `WINDOWS`, the picker, `main()`, `needs_instrument_lock()` | `main.py`, and the `smu-lab-suite` console script | the application would have no importable entry point. A console script must name `module:function`, and pointing one at `main.py` would install a top-level `main` into site-packages |
| `core/single_instance.py` | machine-wide lock, taken by `main.py` before any window, and given back when the CSV plotter is chosen - it opens no instrument | `main.py` | two copies could each open the same instruments, each believing it controlled the output state. The lock is held by the OS, not written down, so a crash cannot leave it stuck |
