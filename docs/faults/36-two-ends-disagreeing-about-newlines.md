...
**The generated documents.** A rebuild on a Windows bench machine left
every generated page showing as modified with no content change - once,
enough to block a `git switch`. Measured on the tree at the time: a
regenerated generated page had **44 CRLF pairs and zero LF bytes**.

**The measurement CSVs.** The code that produced a measurement file and
the file on disk disagreed about its bytes, on every save taken on
Windows.

## Cause

`tools/build_docs.py` wrote every page with
...
Closed. No `schema` bump: the header keys and the column layout are
unchanged, and both forms already existed in the wild depending on the
platform. See [the stored-file schema](../reference/schema.md).

## Evidence

Found while auditing finding A-06.

The CSV decision was a real decision rather than an obvious bug, and it
is worth recording that it was taken rather than defaulted into. RFC
4180 specifies CRLF for CSV, so CRLF on disk was defensible. Three
things settled it:
...
- A file whose bytes depend on the machine cannot be compared.
