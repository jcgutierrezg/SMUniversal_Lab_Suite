...
A check that passes, and would pass equally against code that does
nothing. Nothing is red, and nothing has been established.
...
This is the most-repeated fault in the project's history, and it is the
only one that can hide *all the others*: a test that passes whether or
not the code works leaves every fault beneath it undetected.

It has also appeared as a test asserting state the fake's own reset had
set, a count that reset already satisfied, a mixed-sample guard whose
pre-existing check refused first, and an escape-marker test that
reimplemented the code it was testing.

## Detection

**Mutate and confirm the test can fail.** Break the code the check is
about, on purpose, and watch it go red. A check that survives the
mutation is measuring nothing.

Then ask the fake the same question. An assertion is untestable against
a model that cannot produce a value on the wrong side of it - two fakes
here returned a hardcoded `"false"`, so a correct new probe passed
against fakes that could not have said otherwise.

## Prevention

**Ask at the moment the answer is known and known to be the interesting
one.** `_check_compliance_reported()` in `core/checkup.py` asks while
the instrument is riding its limit, and the fakes compute compliance
from state.

## Status

Recurring by nature. The specific checkup instance is closed.
