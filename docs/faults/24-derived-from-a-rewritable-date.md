...
# A derived claim resting on something a merge rewrites
...
Observed 2026-08-21. `docs/open/checkup-owed.md`, `bench/choosing-an-smu.md`
and one bench page were built saying the GSM-20H10 was commissioned, and
a rebuild minutes later said it needed re-checking. The driver file was
...
somebody merged them. In this case three commits authored on 2026-08-20
were applied by `git am` on 2026-08-21, which moved the driver's date
past the note's `last_bench` and flipped the status.

Two smaller faults were hiding underneath it. The comparison had
day resolution, so a code change made *later the same day* as the bench
...
## Why the local check could not catch it

The clean-checkout verification applies the patch and runs the suite.
`git apply` leaves the files uncommitted, so `git log` still reports the
*pre-patch* date — the local run asked a question whose answer had
already been fixed, and it passed for a reason unrelated to the change.
Committing before running is now the documented procedure, and it is
what caught the second instance. See also
...
Ask whether the generated file would render identically if the same
tree were merged tomorrow instead of today. If any input to it comes
from `git log`, `git rev-parse`, `datetime.now()` or a path outside the
repository, the answer is no.

The test for it is not "render twice and compare" — that passes if both
renders read the same rewritten value. It is to make the rewritable
