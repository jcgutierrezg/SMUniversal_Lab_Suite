...
# 29. A shutdown path that fails open
...
in the middle of recording - and one experiment inherited an
`on_close()` hook that did nothing at all, so its worker was never asked
to stop.
...
- an unconfirmed shutdown produces a modal warning that outlives the
  ...
  [A dialog nobody stubbed](28-a-dialog-nobody-stubbed.md));
- the close path waits, **bounded**, for every worker to reach idle
  before disconnecting anything, pumping the UI queue while it waits - a
  main thread that blocks without draining is waiting on work it is
  itself holding up (see
...
one-line comment naming the invariant that makes it safe - a timer id Tk
has already forgotten cannot fire, so failing to cancel it leaves
nothing scheduled.
[House rule 13](../rules/13-exceptions-are-not-suppressed-silently.md)
states the policy.

## Status

Closed on the named surface. The policy covers fourteen modules, not the
whole tree; the rest is in
