...
# 25. A bound checked on one side only
...
The floor and the ceiling fail in opposite directions, which is why one
tends to get written and the other forgotten:
...
model that cannot produce a value on the wrong side of it.
...
Closed. `COMPLIANCE_FLOOR = 0.8` and `COMPLIANCE_CEILING = 1.25`, both
edges taken from measured hardware, with 1.023× and 2.0× as the two real
readings they have to separate.

## Evidence

Bench session 2026-08-21, U2722A. Same lesson as fault 15 arriving in a
*check* rather than in a driver: the fix was deciding what the boundary
means, not tightening the arithmetic. Applied to a state rather than a
number, it is
