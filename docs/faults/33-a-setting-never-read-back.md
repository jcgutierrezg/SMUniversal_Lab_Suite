...
# A setting reported from the command that was sent
...
- **GSM-20H10, 2026-08-20.** `SENS:CURR:DC:RANG 1.000000e-04` with a
  ...
  is taken on a range the operator did not choose, and overranges into
...
So the question has more than two answers, and a contract that offers
only *agreed* / *disagreed* will be forced to file "nobody asked",
"asked and got nothing", and "asked, and the asking has never been
checked" under one of them. All three get filed under *agreed*, because
that is the one that does not stop the run.

## The rule

**A setting that matters is read back, and the answer has five states,
of which exactly one is a pass.**

`core/readback.py` names them: `unsupported`, `unreadable`,
`unverified`, `confirmed`, `mismatched`. Only `confirmed` renders as a
pass, and it requires both an answer that agrees *and* a bench session
behind the readback itself.
...
This is [A probe asked where the answer is already known](19-non-discriminating-probe.md) applied to a
setting rather than to a measurement, and
[A bound checked on one side only](25-a-bound-checked-on-one-side.md) applied to a state rather than
to a number.
