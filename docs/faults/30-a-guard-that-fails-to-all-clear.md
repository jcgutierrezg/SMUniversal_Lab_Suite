...
# 30. A guard whose own failure reads as all-clear
...
operator presses Save - that is the accepted cost of
[not auto-saving](../rules/03-no-auto-save.md) - so the confirmation
prompt is the only thing standing between a wrong keystroke and a
morning of measuring. A quarter of an hour of instrument time at 200
points and a 2 s settle, per run.

Note also which direction the failures point. Both the count and the
dialog are *more* likely to fail when something else is already wrong -
and "something else is already wrong" is exactly when unsaved data
matters most.
...
must treat "I do not know" as a refusal, never as a pass.** Concretely:
