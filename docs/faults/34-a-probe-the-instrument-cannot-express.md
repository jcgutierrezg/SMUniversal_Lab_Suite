...
# 34. A test level the instrument cannot express
...
The driver refuses that level, correctly, before energising anything. So
the checkup was **structurally unable to pass** on an instrument that
...
output, and this project has now had one of each.

## Detection

Ask of any fixed diagnostic level: *what makes this the right number on
the instrument with the coarsest converter and the narrowest envelope in
the fleet?* If the answer is that it was chosen against a different
instrument, it is a constant waiting to become a failure report.

## Prevention

**A diagnostic sources levels the instrument can express, taken from
that instrument, at the moment the range is known.** The nominal
constants remain, as a starting request. Reconciling them is two steps:

- before anything is sent, clamp into what the model declares
  (`LIMITS`) - a probe above the widest declared range is a request the
  instrument cannot carry out, and every check downstream would then be
  measuring the clamp;
- after the ranging plan has been applied and before the output comes
  up, ask the driver what its floor is *on the range that is now active*
  (`source_level_floor()`), and raise the level to it.
...
Most drivers declare their sub-count behaviour `unmeasured` - the
...
Code closed; the bench fact is open. The 2026-08-25 U2722A report
carries `bench_result: fail` and stands until the checkup is run again
against the hardware. See
[technical debt](../open/technical-debt.md).

## Evidence

Bench session 2026-08-25, U2722A. Deviation 54.
Related: [A probe asked where the answer is already known](19-non-discriminating-probe.md),
