<!-- GENERATED by tools/build_docs.py - do not edit this file.
     Edit the source note in docs/ instead, then
     run: uv run python tools/build_docs.py -->

# Checkup owed

A driver is *commissioned* only while the code that was checked is the code that is running.

This compares a digest of the driver's **contents** against the `bench_code` each note recorded at its last checkup - nobody maintains it, no git history is consulted, and it cannot claim a driver is current when the file has changed.

*failing* is not *stale*. Stale means nobody has checked recently; failing means somebody has, and it did not pass.

| Instrument | Driver | Status | Why |
|---|---|---|---|
| GW Instek GSM-20H10 | `drivers/gwinstek_gsm20h10.py` | stale | the code has changed since the 2026-09-04 checkup |
| Keithley 2401 | `drivers/keithley_2401.py` | stale | the code has changed since the 2026-09-04 checkup |
| Keithley 2611A | `drivers/keithley_2611a.py` | stale | the code has changed since the 2026-09-04 checkup |
| Keithley 2635B | `drivers/keithley_2635b.py` | stale | the code has changed since the 2026-09-04 checkup |
| Keysight B2901A | `drivers/keysight_b2901a.py` | stale | the code has changed since the 2026-09-04 checkup |
| Keysight U2722A | `drivers/keysight_u2722a.py` | stale | the code has changed since the 2026-09-04 checkup |
| Undalogic miniSMU MS01 | `drivers/undalogic_minismu.py` | stale | the code has changed since the 2026-09-04 checkup |

Run `uv run tools/smu_checkup.py --address <addr> --trace`, then copy `last_bench`, `bench_code` and `bench_result` from the report header into the instrument's note and rebuild.

## No checkup is possible

Not owed, and not waiting for anyone. There is no access to these instruments, so no session can be run - the drivers are kept working offline and the rows below will not clear.

| Instrument | Driver | Status | Why |
|---|---|---|---|
| Keithley 2450 | `drivers/keithley_2450.py` | unavailable | there is no access to this instrument - it is not in this lab, and the hardware belongs to another group |
